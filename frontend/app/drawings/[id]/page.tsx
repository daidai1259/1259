...
export default function DrawingViewer(){
 ...
 useEffect(()=>{const id=window.location.pathname.split("/").filter(Boolean).pop(); if(!id)return;
  Promise.all([fetch(`${API}/api/drawings/${id}/page-summaries`).then(r=>r.ok?r.json():Promise.reject()),fetch(`${API}/api/drawings/${id}`).then(r=>r.ok?r.json():Promise.reject())])
   .then(([ps,d])=>{setPages(ps); if(d.pages?.length)setPages(ps); setLoading(false)}).catch(()=>{setError("无法连接审核 API，请确认后端已启动。");setLoading(false)});
 },[]);
 ...
 return <main className="viewer-shell">
  ...
  <div className="viewer-body">
   ...
   <aside className="issue-panel"><div className="panel-head"><h3>问题清单</h3><span>{issues.length} 项</span></div>{issues.map(i=><button key={i.id} className={selected?.id===i.id?"issue-item selected":"issue-item"} onClick={()=>{setSelected(i);const idx=pages.findIndex(p=>p.id===i.page_id);if(idx>=0)setPage(idx+1)}}><span className={"severity "+i.severity}>{i.severity}</span><div><b>{i.title}</b><small>{i.page_id===current.id?"当前页面":"其他页面"}</small></div></button>)}{selected&&<div className="issue-detail"><small>问题详情</small><h4>{selected.title}</h4><p>{selected.description}</p><div className="detail-meta">审核状态 · {selected.status}</div><button className="close-detail" onClick={()=>setSelected(null)}>关闭详情</button></div>}</aside>
  </div>
 </main>
}
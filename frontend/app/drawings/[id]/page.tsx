...
type Issue={id:number;severity:string;title:string;description:string;page_id?:number|null;x0?:number|null;y0?:number|null;x1?:number|null;y1?:number|null;status:string;coordinate_space?:string};

const API=process.env.NEXT_PUBLIC_API_BASE_URL||"http://localhost:8000";

export default function DrawingViewer(){
 const [page,setPage]=useState(1),[zoom,setZoom]=useState(1),[selected,setSelected]=useState<Issue|null>(null),[pendingIssue,setPendingIssue]=useState<number|null>(null);
 ...
 useEffect(()=>{if(!current)return; setSelected(null);
  fetch(`${API}/api/drawings/${current.drawing_id}/issues`).then(r=>r.ok?r.json():[]).then(xs=>{setIssues(xs); const target=pendingIssue==null?null:xs.find((x:Issue)=>x.id===pendingIssue); if(target&&target.page_id===current.id){setSelected(target);setPendingIssue(null);}}).catch(()=>{});
 },[current?.drawing_id]);
 ...
 return <main className="viewer-shell">
  ...
  <div className="viewer-body">
   ...
   <section className="canvas-area"><div className="toolbar"><span>第 {page} / {pages.length} 页</span><button onClick={()=>setZoom(z=>Math.max(.5,z-.1))}>−</button><b>{Math.round(zoom*100)}%</b><button onClick={()=>setZoom(z=>Math.min(2,z+.1))}>＋</button><button onClick={()=>setZoom(1)}>适应</button></div><div className="canvas"><div className="drawing-paper" style={{transform:`scale(${zoom})`}}><img src={`${API}/api/drawing-pages/${current.id}/image`} alt={current.drawing_title||"施工图"} />{pageIssues.map(i=>i.x0!=null&&i.y0!=null&&i.x1!=null&&i.y1!=null&&<button key={i.id} className={selected?.id===i.id?"issue-box selected":"issue-box"} style={{left:`${i.x0*100}%`,top:`${i.y0*100}%`,width:`${(i.x1-i.x0)*100}%`,height:`${(i.y1-i.y0)*100}%`}} onClick={()=>{setSelected(i);setZoom(z=>Math.max(z,1.25))}} aria-label={i.title}/>)}</div></div></section>
   <aside className="issue-panel"><div className="panel-head"><h3>问题清单</h3><span>{issues.length} 项</span></div>{issues.map(i=><button key={i.id} className={selected?.id===i.id?"issue-item selected":"issue-item"} onClick={()=>{const idx=pages.findIndex(p=>p.id===i.page_id);if(idx>=0&&pages[idx].id!==current.id){setPendingIssue(i.id);setPage(idx+1);setZoom(1)}else{setSelected(i)}}}><span className={"severity "+(i.severity==="high"?"高":i.severity==="medium"?"中":"低")}>{i.severity==="high"?"高":i.severity==="medium"?"中":"低"}</span><div><b>{i.title}</b><small>{i.page_id===current.id?"当前页面":"其他页面"}</small></div></button>)}{selected&&<div className="issue-detail"><small>问题详情</small><h4>{selected.title}</h4><p>{selected.description}</p><div className="detail-meta">审核状态 · {selected.status}</div><div className="status-actions"><button onClick={()=>updateIssueStatus(selected.id,"confirmed")}>确认</button><button onClick={()=>updateIssueStatus(selected.id,"fixed")}>已整改</button><button onClick={()=>updateIssueStatus(selected.id,"rejected")}>驳回</button></div><button className="close-detail" onClick={()=>setSelected(null)}>关闭详情</button></div>}</aside>
  </div>
 </main>
}